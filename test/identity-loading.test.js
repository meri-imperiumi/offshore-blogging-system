...
test('should load identity from signalk-reticulum config file (environment-specific)', async (t) => {
  ...
  if (!configData) {
    t.skip('signalk-reticulum config not found - skipping environment-specific test');
    return;
  }

  const config = JSON.parse(configData);

  assert.ok(config.configuration?.identity?.privateKey, 'Config should have identity.privateKey');
  ...
  const publicKeyHex = Buffer.from(await identity.getPublicKey()).toString('hex');
