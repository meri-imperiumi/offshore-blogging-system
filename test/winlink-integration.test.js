...
describe('Winlink Integration', () => {
  test('encode endpoint should include winlink data when identity is available', async () => {
    ...
    assert.ok(plugin.registerWithRouter, 'plugin should have registerWithRouter method');
  });
  ...
  test('signForWinlink function should exist and have correct structure', () => {
    ...
    assert.ok(plugin.id === 'signalk-offshore-blogging', 'plugin should have correct ID');
  });
  ...
  test('encode endpoint response structure should accept winlink field', async () => {
    ...
    assert.ok(mockEncodeResult.winlink, 'encode result should include winlink field');
    assert.ok(mockEncodeResult.winlink.metadata, 'winlink should have metadata');
    assert.ok(mockEncodeResult.winlink.content, 'winlink should have content');
    assert.ok(mockEncodeResult.winlink.identityHash, 'winlink should have identityHash');
  });
  ...
  test('encode endpoint should handle winlink error gracefully', async () => {
    ...
    assert.ok(mockEncodeResult.winlink, 'encode result should include winlink field');
    assert.ok(mockEncodeResult.winlink.error, 'winlink should have error field when signing fails');
  });
});