...
class DatabaseHelper {
  ...
  // Dacar helpers
  saveDacarTuple(issuer, subject, object, relation, expiry) {
    const sql = `
      INSERT OR REPLACE INTO dacar_tuples
      (issuer, subject, object, relation, expiry, created_at)
      VALUES (?, ?, ?, ?, ?, ?)
    `;
    return this.run(sql, [
      issuer,
  ...
  getDacarTuples(subject, object, relation) {
    const sql = `
      SELECT * FROM dacar_tuples
      WHERE subject = ? AND object = ? AND relation = ? AND expiry > ?
    `;
    const now = Math.floor(Date.now() / 1000);
    return this.all(sql, [subject, object, relation, now]);
  }

  saveDacarTombstone(issuer, subject, object, relation, expiresAt) {
    const sql = `
      INSERT OR REPLACE INTO dacar_tombstones
      (issuer, subject, object, relation, created_at, expires_at)
      VALUES (?, ?, ?, ?, ?, ?)
    `;
    return this.run(sql, [
      issuer,
  ...
  getDacarTombstone(subject, object, relation) {
    const sql = `
      SELECT * FROM dacar_tombstones
      WHERE subject = ? AND object = ? AND relation = ? AND expires_at > ?
    `;
    const now = Math.floor(Date.now() / 1000);
    return this.get(sql, [subject, object, relation, now]);
  }

  purgeExpiredDacarTuples() {
    const now = Math.floor(Date.now() / 1000);
    const sql = "DELETE FROM dacar_tuples WHERE expiry < ?";
    return this.run(sql, [now]);
  }

  purgeExpiredDacarTombstones() {
    const now = Math.floor(Date.now() / 1000);
    const sql = "DELETE FROM dacar_tombstones WHERE expires_at < ?";
    return this.run(sql, [now]);
  }
