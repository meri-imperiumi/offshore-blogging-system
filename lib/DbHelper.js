...
class DatabaseHelper {
  ...
  /**
   * Get the part type for a transmission. Used to determine context
   * for alerts (e.g., GRIB vs blog posts).
   */
  getTransmissionPartType(identityHash, transmissionId) {
    const sql = `
      SELECT part_type FROM buffer_chunks
      WHERE identity_hash = ? AND transmission_id = ?
      LIMIT 1
    `;
    const row = this.get(sql, [identityHash, transmissionId]);
    return row ? row.part_type : null;
  }
  ...
  incrementMetric(type, amount = 1) {
    const validTypes = ["blog_posts", "msg_in", "msg_out"];
    if (!validTypes.includes(type)) {
      throw new Error(`Invalid metric type: ${type}`);
    }
    // No-op for zero/negative/non-numeric amounts so callers can pass a
    // message-derived count (e.g. number of acked uids) without guarding.
    const inc = Math.trunc(Number(amount));
    if (!Number.isFinite(inc) || inc <= 0) {
      return;
    }
    const sql = `UPDATE metrics SET ${type} = ${type} + ?, updated_at = ? WHERE id = 1`;
    return this.run(sql, [inc, Math.floor(Date.now() / 1000)]);
  }
