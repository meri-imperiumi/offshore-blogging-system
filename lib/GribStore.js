 ...
 *   <dir>/<sourceName>/          — subdirectory for GRIB files (default: inreach)
 *   <dir>/<sourceName>/<id>.grb  — the binary GRIB file
 *
 * The source subdirectory structure makes this compatible with
 * signalk-grib-weather-provider, which can discover and ingest GRIBs from
 * this directory for querying via the Signal K weather API.
 *
...
class GribStore {
   ...
   * @param {string} [sourceName='inreach'] - Source subdirectory name for GRIB files.
   *                                         This becomes the provider ID suffix in
   *                                         signalk-grib-weather-provider.
   */
  constructor(dir, sourceName = "inreach") {
    this.dir = dir;
    this.sourceName = sourceName;
    this.manifestPath = path.join(dir, "manifest.json");
    this.gribDir = path.join(dir, sourceName);
  }
  ...
  /**
   * Validate a Buffer as a GRIB and return its edition (1 or 2).
   * The GRIB edition byte is at offset 7 (0-indexed); the 4-byte magic
   * `GRIB` is at offset 0.
   *
   * @param {Buffer} binary
   * @returns {{magic:string,edition:number}}
   * @throws {Error} with `.code` of `BAD_MAGIC` (not a GRIB at all) or
   *   `BAD_FORMAT` (too short to read).
   */
  verifyGrib(binary) {
    if (!Buffer.isBuffer(binary) || binary.length < 9) {
      const err = new Error("GRIB payload too short or not a Buffer");
      err.code = "BAD_FORMAT";
      throw err;
    }
    const magic = binary.subarray(0, 4).toString("ascii");
    ...
    // Edition is at byte 8 (0-indexed) in both GRIB1 and GRIB2 — matching
    // the existing download-path MIME detection in the plugin.
    const edition = binary[8] === 2 ? 2 : 1;
    return { magic, edition };
  }

  /**
   * Persist a complete GRIB binary directly (no chunk reassembly).
   *
   * This is the Winlink/Saildocs path: the GRIB arrives as a whole file
   * (e.g. a `.grb` email attachment from query@saildocs.com received via
   * Pat/Winlink), and the user uploads it through the Signal K plugin UI so
   * it lands in the same store — and thus the same
   * signalk-grib-weather-provider source directory — as InReach-assembled
   * GRIBs, making it available to any Signal K user and to the weather API.
   *
   * If `id` is not provided, a stable 4-char Base62 id is derived from a
   * hash of the binary, so uploading the same file twice dedups rather than
   * creating duplicates.
   *
   * @param {Buffer} binary - The full GRIB file contents.
   * @param {object} [opts]
   * @param {string} [opts.id] - Optional 4-char Base62 id. Generated if absent.
   * @param {string} [opts.requestedBy] - Optional identity hash / user label.
   * @param {string} [opts.filename] - Original filename (e.g. from the email
   *   attachment). Stored in the manifest for provenance.
   * @returns {Promise<object>} The manifest entry.
   * @throws {Error} with `.code` of `BAD_MAGIC`, `BAD_FORMAT`, or `BAD_ID`.
   */
  async persistBinary(binary, opts = {}) {
    this.verifyGrib(binary);

    let id = opts.id;
    if (!id) {
      id = this.deriveId(binary);
    }
    if (!ID_RE.test(id)) {
    ...
    await this.ensureDir();
    await fs.mkdir(this.gribDir, { recursive: true });
    const filename = `${id}.grb`;
    const filepath = path.join(this.gribDir, filename);
    await fs.writeFile(filepath, binary);

    const entries = await this.readManifest();
    const existingIdx = entries.findIndex((e) => e.transmissionId === id);
    const now = Date.now();
    ...
    const entry = {
      ...
      filename,
      size: binary.length,
      totalChunks: null, // Direct upload, not chunk-assembled.
      source: "upload", // Distinguishes Winlink/Saildocs uploads from InReach.
      originalFilename: opts.filename || existing?.originalFilename || null,
      createdAt: existing ? existing.createdAt : now,
      updatedAt: now,
      requestedBy: opts.requestedBy || existing?.requestedBy || null,
    };
  ...
  /**
   * Derive a stable 4-char Base62 id from a GRIB binary. Uses a fast,
   * non-cryptographic hash (FNV-1a) so uploading the same file twice resolves
   * to the same id and dedups the manifest entry.
   *
   * @param {Buffer} binary
   * @returns {string} 4-char Base62 id.
   */
  deriveId(binary) {
    ...
    // 4-char Base62, ~1.3e7 values. Collisions on a boat are unlikely; the
    // persist path overwrites by id, so a worst-case collision just replaces
    // a file with identical content.
    const BASE62 =
  ...
  getFilePath(id) {
    ...
    return path.join(this.gribDir, `${id}.grb`);
  }
}
