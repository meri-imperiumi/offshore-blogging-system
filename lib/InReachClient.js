    ...
    const req = https.request(reqOptions, (res) => {
      ...
      if (
        ...
        if (depth > MAX_REDIRECTS) {
          reject(
            new InReachError("Too many redirects fetching reply page", "BAD_URL"),
          );
          return;
        }
        resolve(
          httpRequest(next, { ...options, _redirectDepth: depth }),
        );
        return;
      }
      const chunks = [];
...
async function requestWithRetry(
  ...
  for (let attempt = 0; attempt <= maxRetries; attempt++) {
    try {
      ...
      if (
        isRetryableStatus(lastResponse.status) &&
        attempt < maxRetries
      ) {
...
class InReachClient {
  ...
  async send(replyUrl, message) {
    ...
    const messageId =
      (formValues && formValues.messageId) || randomMessageId();
